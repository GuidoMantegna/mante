...
```markdown
## R1
CUANDO el usuario envía el formulario con un texto no vacío, el sistema
DEBE agregar una nueva tarea al final de la lista.

## R2
SI el usuario envía el formulario con el campo de texto vacío ENTONCES
el sistema NO DEBE agregar ninguna tarea a la lista.
```
...
```markdown
- [ ] T1 — Crear `components/task-form.tsx` con input controlado y botón submit. Cubre: R1, R2.
- [ ] T2 — Integrar `TaskForm` en `app/page.tsx`, manteniendo la lista en `useState`. Cubre: R1.
- [ ] T3 — Añadir `tests/task-form.test.tsx` con el caso "agrega tarea con texto válido". Cubre: R1.
- [ ] T4 — Añadir a `tests/task-form.test.tsx` el caso "no agrega tarea con texto vacío". Cubre: R2.
```
